...
(define (min-tt-acc counters acc)
  (if (null? counters)
      acc
      (if (< (counter-tt (car counters)) (cdr acc))
          (min-tt-acc (cdr counters) (cons (counter-index (car counters)) (counter-tt (car counters))))
          (min-tt-acc (cdr counters) acc)
      )
   )
)

(define (min-tt counters)
  (min-tt-acc counters (cons (counter-index (car counters)) (counter-tt (car counters))))
)
  ...
  (if (null? requests)
      ...
      (match (car requests)
        ...
        [(list name n-items)
         (if (> n-items ITEMS)
             (cond
               ((= 2 (car(min-tt (cdr counters)))) (serve (cdr requests) C1 (add-to-counter C2 name n-items) C3 C4))
               ((= 3 (car(min-tt (cdr counters)))) (serve (cdr requests) C1 C2 (add-to-counter C3 name n-items) C4))
               ((= 4 (car(min-tt (cdr counters)))) (serve (cdr requests) C1 C2 C3 (add-to-counter C4 name n-items)))
             )
             (cond
               ((= 1 (car(min-tt counters))) (serve (cdr requests) (add-to-counter C1 name n-items) C2 C3 C4))
               ((= 2 (car(min-tt counters))) (serve (cdr requests) C1 (add-to-counter C2 name n-items) C3 C4))
               ((= 3 (car(min-tt counters))) (serve (cdr requests) C1 C2 (add-to-counter C3 name n-items) C4))
               ((= 4 (car(min-tt counters))) (serve (cdr requests) C1 C2 C3 (add-to-counter C4 name n-items)))
             )
         )
        ]
      )
