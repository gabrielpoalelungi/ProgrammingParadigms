...
; MIN-TT AND MIN-ET WITH GENERAL FUNCTION
(define (get-tt C)
	(counter-tt C)
)
(define (get-et C)
...
(define (serve-aux requests fast-counters slow-counters acc)
  (if (null? requests)
      (append (list acc) (append fast-counters slow-counters))
      (match (car requests)
        [(list 'ensure average)
        ...
        [(list name items)
         (if (<= items ITEMS)
             (serve-aux (cdr requests)
                    (update (add-to-counter name items) fast-counters (car (min-tt (append fast-counters slow-counters))))
                    (update (add-to-counter name items) slow-counters (car (min-tt (append fast-counters slow-counters))))
                    acc
             )
             (serve-aux (cdr requests)
                    fast-counters
                    (update (add-to-counter name items) slow-counters (car (min-tt slow-counters)))
                    acc
             )
         )
        ]
