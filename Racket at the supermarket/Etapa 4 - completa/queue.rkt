...
(define (enqueue x q)
  (if (= (stream-length (queue-left q)) (length (queue-right q)))
      (struct-copy queue q
               [left (rotate (queue-left q) (append (list x) (queue-right q)) empty-stream)]
               [right null]
               [size-r 0]
               [size-l (+ (stream-length (queue-left q)) (+ (length (queue-right q)) 1))]
      )
      (struct-copy queue q
...
(define (dequeue q)
  (if (= (stream-length (queue-left q)) (length (queue-right q)))
      (struct-copy queue q
               ...
               [right null]
               [size-r 0]
               [size-l (+ (- (stream-length (queue-left q)) 1) (length (queue-right q)))]
      )
      (struct-copy queue q
